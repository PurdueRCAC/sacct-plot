...
class TestJobInfo:
    ...
    def test_basic_job(self):
        line = '12345|user1|account1|normal|8|billing=8,cpu=8,mem=64G|3600|COMPLETED|2026-02-18T09:30:00|2026-02-18T10:00:00|2026-02-18T11:00:00'
        ...
        assert job.submit == datetime(2026, 2, 18, 9, 30, 0)
        assert job.start == datetime(2026, 2, 18, 10, 0, 0)
        assert job.end == datetime(2026, 2, 18, 11, 0, 0)
        assert job.gpus == 0

    def test_gpu_job(self):
        line = '67890|user2|gpu_account|gpu|32|billing=32,cpu=32,gres/gpu=4,mem=256G|7200|COMPLETED|2026-02-18T07:00:00|2026-02-18T08:00:00|2026-02-18T10:00:00'
        job = JobInfo.from_line(line)
        assert job.ncpus == 32
        assert job.gpus == 4
        assert job.elapsed_raw == 7200
        assert job.submit == datetime(2026, 2, 18, 7, 0, 0)

    def test_array_job_id(self):
        """Array job IDs with underscores should be split at the dot."""
        line = '12345_10|user1|acct|normal|4|cpu=4|1800|COMPLETED|2026-02-18T09:30:00|2026-02-18T10:00:00|2026-02-18T10:30:00'
        job = JobInfo.from_line(line)
        assert job.job_id == '12345_10'

    def test_step_job_id(self):
        """Job step IDs (12345.batch) should be truncated to the base job ID."""
        line = '12345.batch|user1|acct|normal|4|cpu=4|1800|COMPLETED|2026-02-18T09:30:00|2026-02-18T10:00:00|2026-02-18T10:30:00'
        job = JobInfo.from_line(line)
        assert job.job_id == '12345'

    def test_pending_job_no_end(self):
        line = '99999|user1|acct|normal|16|cpu=16|0|PENDING|2026-02-18T09:00:00|Unknown|Unknown'
        job = JobInfo.from_line(line)
        assert job.submit == datetime(2026, 2, 18, 9, 0, 0)
    ...
    def test_to_dict(self):
        line = '12345|user1|acct|normal|8|billing=8,cpu=8,gres/gpu=2,mem=64G|3600|COMPLETED|2026-02-18T09:30:00|2026-02-18T10:00:00|2026-02-18T11:00:00'
        job = JobInfo.from_line(line)
        d = job.to_dict()
        ...
        assert d['submit'] == datetime(2026, 2, 18, 9, 30, 0)
